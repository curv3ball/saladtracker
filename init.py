...
def create_viewport(title: str = "Viewport_1") -> None:
...
def create_images() -> None:
    ...
    img.open(
        io.BytesIO(
            base64.b64decode(salad_logo_bytes)
            )
        ).save(os.path.join(temp, "salad_logo.png")
    )
...
def setup_theme() -> None:
    ...
    dearpygui.bind_item_theme(window_settings.WINDOW_TAG, global_theme)

def setup_window() -> None:
    """https://dearpygui.readthedocs.io/en/latest/documentation/primary-window.html"""
    dearpygui.show_viewport()
    dearpygui.set_primary_window(window_settings.WINDOW_TAG, True)
